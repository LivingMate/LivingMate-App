...
const RegisterEventModalContainer: React.FC<RegisterEventModalContainerProps> = ({ mode, getEvents, editingEvent, isVisible, onClose }) => {
  ...
  const { userToken, canUpdateTodos } = useAuth();
  ...
  const addEvent = async (title: string, start: Date, end: Date, term: number, participants: UserProps[], memo: string) => {
    // JSON 데이터 생성
  ...
  const editEvent = async (title: string, start: Date, end: Date, term: number, participants: UserProps[], memo: string) => {
      try {
        const participantIds = participants.map(user => user.userId);
        const newData = {
          title: title, 
